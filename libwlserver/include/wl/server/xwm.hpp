...
struct XWindow {
    uint32_t              xcb_id   = 0;
    uint32_t              admin_id = 0;
    wl::server::SurfaceId surface_id;
...
class XwmSurfaceSink {
    public:
        ...
        virtual void     set_surface_wl_id(uint32_t id, wl::server::SurfaceId sid)      = 0;
...
class XWindowManager : protected xcb::Connection {
    public:
        ...
        void configure(uint32_t admin_id, int32_t x, int32_t y, int32_t w, int32_t h);
        void close(uint32_t admin_id);
        void activate(uint32_t admin_id, bool activated);
        bool owns(uint32_t admin_id) const;
    ...
    private:
        ...
        std::unordered_set<uint32_t>          admin_ids_;
        ...
        XWindow* window_by_admin_id(uint32_t admin_id);
};
