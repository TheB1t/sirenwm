...
class XWayland {
    public:
        ...
        void configure_window(uint32_t admin_id, int32_t x, int32_t y, int32_t w, int32_t h);
        void close_window(uint32_t admin_id);
        void activate_window(uint32_t admin_id, bool activated);

        int  xcb_fd() const;
        void dispatch();

        bool is_xwayland_surface(uint32_t admin_id) const;
