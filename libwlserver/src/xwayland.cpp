...
bool XWayland::open_display_sockets() {
    for (int i = 0; i < 32; i++) {
        ...
        snprintf(path1, sizeof(path1), "@/tmp/.X11-unix/X%d", i);
...
void XWayland::configure_window(uint32_t admin_id, int32_t x, int32_t y,
    int32_t w, int32_t h) {
    if (xwm_) xwm_->configure(admin_id, x, y, w, h);
}

void XWayland::close_window(uint32_t admin_id) {
    if (xwm_) xwm_->close(admin_id);
}

void XWayland::activate_window(uint32_t admin_id, bool activated) {
    if (xwm_) xwm_->activate(admin_id, activated);
}

bool XWayland::is_xwayland_surface(uint32_t admin_id) const {
    return xwm_ && xwm_->owns(admin_id);
}
