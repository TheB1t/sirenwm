...
void XWindowManager::handle_map_request(xcb_map_request_event_t* ev) {
    ...
    if (xwin.wl_surface_id) {
        xwin.surface_id = sink_.add_surface(xwin.wm_class, xwin.title, xwin.pid);
        managed_surface_ids_.insert(xwin.surface_id);
        sink_.set_wl_surface_id(xwin.surface_id, xwin.wl_surface_id);
        sink_.surface_mapped(xwin.surface_id);
        sink_.surface_committed(xwin.surface_id, xwin.width, xwin.height);
    }
}
...
void XWindowManager::handle_unmap_notify(xcb_unmap_notify_event_t* ev) {
    ...
    if (xwin.surface_id != 0) {
        sink_.surface_destroyed(xwin.surface_id);
        managed_surface_ids_.erase(xwin.surface_id);
        xwin.surface_id = 0;
    }
...
void XWindowManager::handle_destroy_notify(xcb_destroy_notify_event_t* ev) {
    ...
    if (it->second.surface_id != 0) {
        sink_.surface_destroyed(it->second.surface_id);
        managed_surface_ids_.erase(it->second.surface_id);
    }
...
void XWindowManager::handle_property_notify(xcb_property_notify_event_t* ev) {
    ...
    if (ev->atom == atoms_[NET_WM_NAME] || ev->atom == XCB_ATOM_WM_NAME) {
        read_title(xwin);
        if (xwin.surface_id != 0)
            sink_.surface_title_changed(xwin.surface_id, xwin.title);
    } else if (ev->atom == atoms_[WM_CLASS_ATOM]) {
        read_class(xwin);
        if (xwin.surface_id != 0)
            sink_.surface_app_id_changed(xwin.surface_id, xwin.wm_class);
    }
}
...
void XWindowManager::associate(XWindow& xwin, wl::server::SurfaceId sid) {
    xwin.wl_surface_id = sid;

    if (xwin.mapped && xwin.surface_id == 0) {
        xwin.surface_id = sink_.add_surface(xwin.wm_class, xwin.title, xwin.pid);
        managed_surface_ids_.insert(xwin.surface_id);
        sink_.set_wl_surface_id(xwin.surface_id, sid);
        sink_.surface_mapped(xwin.surface_id);
        sink_.surface_committed(xwin.surface_id, xwin.width, xwin.height);
    } else if (xwin.surface_id != 0) {
        sink_.set_wl_surface_id(xwin.surface_id, sid);
    }
}
...
XWindow* XWindowManager::window_by_surface_id(uint32_t surface_id) {
    for (auto& [_, xwin] : windows_)
        if (xwin.surface_id == surface_id) return &xwin;
    return nullptr;
}

void XWindowManager::configure(uint32_t surface_id, int32_t x, int32_t y,
                                int32_t w, int32_t h) {
    auto* xwin = window_by_surface_id(surface_id);
...
void XWindowManager::close(uint32_t surface_id) {
    auto* xwin = window_by_surface_id(surface_id);
    if (!xwin || !conn_) return;
...
void XWindowManager::activate(uint32_t surface_id, bool activated) {
...
bool XWindowManager::owns(uint32_t surface_id) const {
    return managed_surface_ids_.count(surface_id) > 0;
}
