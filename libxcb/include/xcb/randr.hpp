...
public:
    ...
    int output_count() const {
        auto r = reply(xcb_randr_get_crtc_info_reply(conn_,
                 xcb_randr_get_crtc_info(conn_, id, XCB_CURRENT_TIME), nullptr));
        return r ? xcb_randr_get_crtc_info_outputs_length(r.get()) : -1;
    }

    int16_t          x()        const { return info_() ? info_cached_->x        : 0; }
    int16_t          y()        const { return info_() ? info_cached_->y        : 0; }
    xcb_randr_mode_t mode()     const { return info_() ? info_cached_->mode     : static_cast<xcb_randr_mode_t>(XCB_NONE); }
    uint16_t         rotation() const { return info_() ? info_cached_->rotation : static_cast<uint16_t>(XCB_RANDR_ROTATION_ROTATE_0); }

    uint8_t configure_status(int16_t x, int16_t y, xcb_randr_mode_t mode,
                             uint16_t rotation, xcb_randr_output_t output) {
        auto r = reply(xcb_randr_set_crtc_config_reply(conn_,
                 xcb_randr_set_crtc_config(conn_, id, XCB_CURRENT_TIME, XCB_CURRENT_TIME,
                 x, y, mode, rotation, 1, &output), nullptr));
...
private:
    mutable xcb_randr_get_crtc_info_reply_t* info_cached_ = nullptr;
    bool info_() const {
        if (info_cached_) return true;
        info_cached_ = xcb_randr_get_crtc_info_reply(conn_,
                       xcb_randr_get_crtc_info(conn_, id, XCB_CURRENT_TIME), nullptr);
        return info_cached_ != nullptr;
    }
};
...
public:
    ...
    bool connected() const {
        auto r = reply(xcb_randr_get_output_info_reply(conn_,
    ...
    std::vector<Mode> modes() const {
        auto res = reply(xcb_randr_get_screen_resources_current_reply(conn_,
                  xcb_randr_get_screen_resources_current(conn_, root), nullptr));
        auto oi = reply(xcb_randr_get_output_info_reply(conn_,
                 xcb_randr_get_output_info(conn_, id, XCB_CURRENT_TIME), nullptr));
        if (!res || !oi) return {};
        ...
        for (int i = 0; i < noms; i++)
            for (int j = 0; j < nall; j++) {
                ...
                double rate = (all[j].htotal && all[j].vtotal)
                    ? static_cast<double>(all[j].dot_clock) / (static_cast<double>(all[j].htotal) * all[j].vtotal)
                    : 0.0;
    ...
    std::optional<Mode> find_mode(int want_w, int want_h, int want_rate) const {
        ...
        for (auto& m : modes()) {
            if (want_w > 0 && (m.width != static_cast<uint16_t>(want_w) || m.height != static_cast<uint16_t>(want_h)))
                continue;
...
public:
    ...
    std::vector<MonitorInfo> monitors() const {
        auto r = reply(xcb_randr_get_monitors_reply(conn_,
        ...
        while (it.rem > 0) {
            ...
            auto nr = reply(xcb_get_atom_name_reply(conn_,
            ...
            result.push_back({id++, std::move(name), info->x, info->y, info->width, info->height});
