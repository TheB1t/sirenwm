...
// Returns a single uint32_t cardinal property, or 0 on failure.
inline uint32_t get_cardinal_property(xcb_connection_t* conn, xcb_window_t win,
    xcb_atom_t prop) {
    auto r = xcb::reply(xcb_get_property_reply(conn,
            xcb_get_property(conn, 0, win, prop, XCB_ATOM_CARDINAL, 0, 1), nullptr));
    if (!r || xcb_get_property_value_length(r.get()) < 4) return 0;
    return *static_cast<uint32_t*>(xcb_get_property_value(r.get()));
}
